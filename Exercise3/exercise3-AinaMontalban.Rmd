...
The same quality check we have done with the data, we can do it with the Normalized data.
...
```{r}
res <- decideTests(fit.main1, method = "separate", adjust.method = "fdr", p.value = 0.1, lfc = 1)
...
```{r}
listOfTables <- list(CD.vs.HF = topTab_CDvsHF, 
                     HF.vs.HF_RES  = topTab_HFvsHF_RES, 
                     CD.vs.HF_RES = topTab_CDvsHF_RES)
listOfSelected <- list()
...
```{r}
EntrezUni <- topTab.end1$ENTREZID
